...
__export(extension_exports, {
  ...
  defaultAdjustTopPosition: () => defaultAdjustTopPosition,
...
var defaultIconWidth = 64;
var defaultIconHeight = 64;
...
var getIconPath = (iconPathSetting, width, height) => {
  ...
  if (supportedFormats.includes(ext)) {
    if (ext === ".svg") {
      try {
        const svgContent = import_fs.default.readFileSync(iconPathSetting, "utf8");
        ...
        const tempSvgPath = import_path.default.join(tempDir, `icon_tmp.svg`);
        import_fs.default.writeFileSync(tempSvgPath, modifiedSvgContent, "utf8");
        return tempSvgPath;
      } catch (error) {
    ...
    try {
      ...
      const tempDir = import_os.default.tmpdir();
      const tempSvgPath = import_path.default.join(tempDir, `icon_tmp.svg`);
      import_fs.default.writeFileSync(tempSvgPath, svgContent, "utf8");
      return tempSvgPath;
    } catch (error) {
...
function activate(context) {
  ...
  function createDecorationType() {
    const isEnabled = vscode.workspace.getConfiguration().get("rustPanicHighlighter.icon.enabled") ?? true;
    if (!isEnabled) {
      return vscode.window.createTextEditorDecorationType({});
    }
    let iconPathSetting = vscode.workspace.getConfiguration().get("rustPanicHighlighter.icon.path") || "default";
    if (iconPathSetting === "default") {
      iconPathSetting = context.asAbsolutePath(defaultIconPath);
    }
    const iconWidthSetting = vscode.workspace.getConfiguration().get("rustPanicHighlighter.icon.width") || defaultIconWidth;
    const iconHeightSetting = vscode.workspace.getConfiguration().get("rustPanicHighlighter.icon.height") || defaultIconHeight;
    const adjustTopPosition = vscode.workspace.getConfiguration().get("rustPanicHighlighter.icon.adjustTopPosition") || defaultAdjustTopPosition;
    if (iconWidthSetting !== void 0 && iconWidthSetting <= 0) {
      throw new Error("icon.width must be a positive number.");
    }
    if (iconHeightSetting !== void 0 && iconHeightSetting <= 0) {
    ...
    const iconsize = (0, import_image_size.default)(iconPath_tmp);
  ...
  vscode.workspace.onDidChangeConfiguration((event) => {
    if (event.affectsConfiguration("rustPanicHighlighter.icon.enabled") || event.affectsConfiguration("rustPanicHighlighter.icon.path") || event.affectsConfiguration("rustPanicHighlighter.icon.width") || event.affectsConfiguration("rustPanicHighlighter.icon.height") || event.affectsConfiguration("rustPanicHighlighter.icon.adjustTopPosition")) {
...
function updateDiagnostics(doc, diagnosticCollection, decorationType) {
  ...
  const severitySetting = vscode.workspace.getConfiguration().get("rustPanicHighlighter.diagnostic.severity");
  let severity;
