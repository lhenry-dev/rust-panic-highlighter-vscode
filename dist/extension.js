  ...
  if (supportedFormats.includes(ext)) {
    if (ext === ".svg") {
      try {
        ...
        const tempSvgPath = import_path.default.join(tempDir, `${tmpFileName}_${uniqueId}.svg`);
        import_fs.default.writeFileSync(tempSvgPath, modifiedSvgContent, "utf8");
        tempFiles.push(tempSvgPath);
        if (!import_fs.default.existsSync(tempSvgPath)) {
...
function activate(context) {
  ...
  function createDecorationType() {
    ...
    const iconSize = vscode.workspace.getConfiguration().get("rustPanicHighlighter.icon.size") || defaultIconSize;
  ...
  vscode.workspace.onDidChangeConfiguration((event) => {
    if (event.affectsConfiguration("rustPanicHighlighter.icon.enabled") || event.affectsConfiguration("rustPanicHighlighter.icon.path") || event.affectsConfiguration("rustPanicHighlighter.icon.size") || event.affectsConfiguration("rustPanicHighlighter.icon.adjustTopPosition")) {
      decorationType.dispose();
      tempFiles.forEach((file) => {
        import_fs.default.promises.unlink(file);
      });
      tempFiles = [];
      decorationType = createDecorationType();
      context.subscriptions.push(decorationType);
...
function updateDiagnostics(doc, diagnosticCollection, decorationType2) {
  let diagnostics = [];
  ...
  if (editor) {
    if (rangesToDecorate.length > 0) {
      editor.setDecorations(decorationType2, rangesToDecorate);
    } else {
      editor.setDecorations(decorationType2, []);
    }
  }
