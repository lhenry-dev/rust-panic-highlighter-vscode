...
var createTempSvgPath = (content) => {
  const tempDir = import_os.default.tmpdir();
  const rustPanicHighlighterDir = import_path.default.join(tempDir, tmpFolderName);
  if (import_fs.default.existsSync(rustPanicHighlighterDir)) {
    const files = import_fs.default.readdirSync(rustPanicHighlighterDir);
    files.forEach((file) => {
      const filePath = import_path.default.join(rustPanicHighlighterDir, file);
      if (import_fs.default.lstatSync(filePath).isFile()) {
        import_fs.default.unlinkSync(filePath);
      }
    });
  } else {
    import_fs.default.mkdirSync(rustPanicHighlighterDir);
  }
...
function createDecorationType(context) {
  ...
  let iconPathSetting = vscode2.workspace.getConfiguration().get("rustPanicHighlighter.icon.path") || "default";
...
function activate(context) {
  let diagnosticCollection = vscode4.languages.createDiagnosticCollection("rust-panic-highlighter");
